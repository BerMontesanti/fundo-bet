...
TARGET_EV = 0.05    # 5.0% ROI
TARGET_EDGE = 0.025 # 2.5% Edge
...
EMAILS_DESTINO = ["bernardo.montesanti@gmail.com", "eduasy@hotmail.com"]
...
def enviar_email(dados_aprovados):
    ...
    if dados_aprovados:
        ...
        tabela_html = df.to_html(index=False, justify='center', border=1)
        
        msg["Subject"] = f"🔥 Alerta +EV: {len(df)} Oportunidades ({data_atual})"
        corpo = f"<html><body><h2>🤖 Alerta Quant: {len(df)} Oportunidades!</h2>{tabela_html}<br><p>Finalizada em {data_atual} às {hora_atual}</p></body></html>"
    else:
        msg["Subject"] = f"💤 Alerta Quant: Nenhuma Oportunidade ({data_atual})"
        corpo = f"<html><body><h2>🤖 Alerta Quant: Zero Oportunidades</h2><p>Varredura de {data_atual} às {hora_atual} concluída. Mercado ajustado no momento.</p></body></html>"
