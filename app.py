...
def calc_roi_realizado(row):
    if row['Status_Aposta'] in ['Green ✅', 'Red ❌'] and row['Stake_Final'] > 0:
        return row['Lucro'] / row['Stake_Final']
    return 0.0
...
with tab_dash:
    ...
    if not df_resolvidas.empty:
        ...
        with col_g1:
            ...
            fig_fin = px.line(
                grafico_dados, x='Data_Curta', y=['Stake Acumulada', 'Lucro Acumulado'], 
                markers=True, labels={'value': 'Valor (R$)', 'variable': 'Métrica', 'Data_Curta': 'Data'}
            )
            fig_fin.for_each_trace(lambda t: t.update(name={'Stake Acumulada': 'Stake Total', 'Lucro Acumulado': 'Lucro'}.get(t.name, t.name)))
...
with tab_hist:
    ...
    cols_display = ['Data/Hora', 'Liga', 'Jogo', 'Casa', 'Seleção', 'Odd Justa', 'Odd Casa', 'Edge', 'ROI', 'Stake', 'Lucro', 'Status_Aposta', 'Aposta_Realizada', 'Vencedor_Partida']
